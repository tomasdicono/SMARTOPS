...
export async function downloadDailyOtpPdf(
    rows: Flight[],
    dateLabel: string,
    planDeAccionMap: Record<string, string>
): Promise<void> {
    ...
    doc.text("DAILY OTP", pageW / 2, 17, { align: "center" });
    ...
    const body = rows.map((f) => {
        ...
        const plan = (planDeAccionMap[f.id] || "").trim();
        return [
            ttl,
    ...
    const dateFormatted = dateLabel.split("-").reverse().join("-");
    doc.save(`Daily Otp_${dateFormatted}_AR.pdf`);
}
