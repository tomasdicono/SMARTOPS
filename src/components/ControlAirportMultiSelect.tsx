...
export function ControlAirportMultiSelect({
    ...
    const [panelRect, setPanelRect] = useState<{ top: number; left: number; width: number; maxHeight: number } | null>(
        null,
    );
    ...
    const updatePanelRect = () => {
        ...
        const maxHeight = Math.min(208, Math.max(96, preferBelow ? spaceBelow : spaceAbove));
            ...
            ? createPortal(
                  <>
                      ...
                      <div
                          ...
                          className="overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-xl py-1 [scrollbar-width:thin]"
                      >
                          {options.length === 0 ? (
        ...
        <div className={`relative shrink-0 min-w-[11rem] max-w-[16rem] ${className}`}>
            ...
            {selected.length > 0 ? (
                <button
                    type="button"
                    onClick={() => onChange([])}
                    className="absolute -top-0.5 -right-0.5 p-0.5 rounded-full bg-slate-200 hover:bg-slate-300 text-slate-700 z-10"
