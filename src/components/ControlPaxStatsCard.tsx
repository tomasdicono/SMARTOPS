...
import { computeTopAvgPaxGroups, getMvtPaxOnly } from "../lib/controlHelpers";
...
function formatAvgPax(n: number): string {
    return Math.round(n).toLocaleString("es-AR");
}

export function ControlPaxStatsCard({ flights, selectedAirports }: Props) {
    const [expanded, setExpanded] = useState(false);
    const totalPax = useMemo(() => flights.reduce((s, f) => s + getMvtPaxOnly(f), 0), [flights]);
    const { mode, rows } = useMemo(
        () => computeTopAvgPaxGroups(flights, selectedAirports, 5),
    ...
    const panelTitle =
        mode === "routes"
            ? "Top 5 — mayor promedio de pasajeros por ruta"
            : "Top 5 — mayor promedio de pasajeros por destino";
    const hint =
        mode === "routes"
            ? "Clic para ver top 5 promedio de pasajeros por ruta"
            : "Clic para ver top 5 promedio de pasajeros por destino";

    return (
        <div
            className={`rounded-xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white overflow-hidden ${
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="bg-slate-100 text-left text-xs font-black uppercase tracking-wider text-slate-600">
                                        ...
                                        <th className="px-3 py-2 text-right">Total PAX</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {rows.map((row, idx) => (
